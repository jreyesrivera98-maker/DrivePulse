-- ==========================================================
-- 0008_incidencia_fotos.sql
--
-- Agrega soporte para adjuntar fotos generales (no ligadas a una
-- zona específica del mapa de desperfectos) al campo de
-- Incidencias/Observaciones de la bitácora.
-- ==========================================================

alter table bitacoras add column if not exists incidencia_fotos jsonb default '[]'::jsonb;

-- Se reemplaza submit_bitacora para aceptar el nuevo parámetro
-- p_incidencia_fotos. El signature cambia (parámetro nuevo), así que
...
grant execute on function public.submit_bitacora(
  uuid, text, text, text, text, int, int, fuel_level, fuel_level,
  boolean, text, jsonb, text, double precision, double precision, jsonb, text, jsonb
) to authenticated;
